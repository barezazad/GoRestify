...
func InitAccAccountCreditAPI(e *core.Engine) acc_api.AccountCreditAPI {
	accountCreditRepo := acc_repo.ProvideAccountCreditRepo(e)
	accAccountCreditServ := service.ProvideAccAccountCreditService(accountCreditRepo)
	accountCreditAPI := acc_api.ProvideAccountCreditAPI(accAccountCreditServ)
	return accountCreditAPI
}

func InitBaseDocumentAPI(e *core.Engine) base_api.DocumentAPI {
	documentRepo := base_repo.ProvideDocumentRepo(e)
	baseDocumentServ := service.ProvideBaseDocumentService(documentRepo)
	documentAPI := base_api.ProvideDocumentAPI(baseDocumentServ)
	return documentAPI
}
