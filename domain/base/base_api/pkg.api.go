...
// LoginBlockList lists IPs currently tracked/blocked by login rate limiter.
func (a *PkgAPI) LoginBlockList(c *gin.Context) {
	resp, _ := response.NewParam(c, models.SettingTable)

	list := middleware.ListLoginBlocks()
	data := map[string]interface{}{
		"list":  list,
		"count": len(list),
	}

	resp.Record(base.ListLoginBlock)
	resp.Status(http.StatusOK).
		Message(pkg_terms.ListOfV, pkg_terms.LoginBlocks).
		JSON(data)
}

type unblockLoginPayload struct {
	IP string `json:"ip" bind:"required"`
}

// LoginUnblock clears progressive login lockout for an IP.
func (a *PkgAPI) LoginUnblock(c *gin.Context) {
	resp, _ := response.NewParam(c, models.SettingTable)

	var payload unblockLoginPayload
	if err := resp.Bind(&payload, "E1194302", pkg_terms.IP); err != nil {
		return
	}

	middleware.UnblockLoginIP(payload.IP)

	resp.Record(base.UnblockLogin, payload.IP)
	resp.Status(http.StatusOK).
		Message(pkg_terms.VUnblockedSuccessfully, payload.IP).
		JSON(payload)
}
