...
// GetAll of currencies without filter, order, or pagination
func (s *AccCurrencyServ) GetAll(params param.Param) (currencies []acc_model.Currency, err error) {
	...
	if currencies, err = s.Repo.GetAll(); err != nil {
...
// Delete currency — hard delete; blocked while referenced by ledger tables.
func (s *AccCurrencyServ) Delete(txSession tx.Tx, id uint) (currency acc_model.Currency, err error) {

	if currency, err = s.FindByID(txSession, id); err != nil {
		pkg_err.Log(err, "E1150148", "currency not found for deleting")
		return
	}

	var creditCount, txCount, slotCount int64
	_ = txSession.GetDB(s.Engine.DB).Table(acc_model.AccountCreditTable).Where("currency_id = ?", id).Count(&creditCount)
	_ = txSession.GetDB(s.Engine.DB).Table(acc_model.TransactionTable).Where("currency_id = ?", id).Count(&txCount)
	_ = txSession.GetDB(s.Engine.DB).Table(acc_model.SlotTable).Where("currency_id = ?", id).Count(&slotCount)
	if creditCount > 0 || txCount > 0 || slotCount > 0 {
		err = pkg_err.New(acc_term.CurrencyInUse, "E1201040").
			Message(acc_term.CurrencyInUse).Custom(pkg_err.ValidationFailedErr).Build()
		return
	}

	if err = s.Repo.Delete(txSession, currency); err != nil {
