...
func generatePayload(request *Request) (payload io.Reader, err error) {
	...
	if request.FormData.FilePath != "" { // if payload is DataForm
		...
		file, openErr := os.Open(request.FormData.FilePath)
		if openErr != nil {
			err = pkg_err.Take(openErr, "E1168268", "error opening multipart file").
				Message(pkg_err.SomethingWentWrong).Custom(pkg_err.InternalServerErr).Build()
			return
		}
		defer file.Close()

		part, partErr := writer.CreateFormFile(request.FormData.FileKey, filepath.Base(request.FormData.FilePath))
		if partErr != nil {
			err = pkg_err.Take(partErr, "E1168269", "error in set file in payload").
				Message(pkg_err.SomethingWentWrong).Custom(pkg_err.InternalServerErr).Build()
			return
		}
		if _, copyErr := io.Copy(part, file); copyErr != nil {
			err = pkg_err.Take(copyErr, "E1168270", "error copying multipart file").
				Message(pkg_err.SomethingWentWrong).Custom(pkg_err.InternalServerErr).Build()
			return
		}
		...
		if closeErr := writer.Close(); closeErr != nil {
			err = pkg_err.Take(closeErr, "E1127211", "error in close write file").
				Message(pkg_err.SomethingWentWrong).Custom(pkg_err.InternalServerErr).Build()
			return
		}
