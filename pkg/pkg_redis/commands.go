...
// KeysPattern to get all keys by pattern using SCAN (non-blocking vs KEYS).
// example key = "customer-*"
func (redisCon RedisCon) KeysPattern(pattern string) (keys []string) {

	if DisplayLog {
		fmt.Println("Redis SCAN Pattern:", pattern)
	}

	var cursor uint64
	for {
		batch, next, err := redisCon.client.Scan(Ctx, cursor, pattern, 500).Result()
		if err != nil {
			return
		}
...
// example keyPattern = "customer-*" or "customer-"
func (redisCon RedisCon) ResetCacheByKeyPatten(keyPattern string) {
	...
	for i := 0; i < len(keys); i += batchSize {
		...
		_ = redisCon.DeleteMultiKeys(keys[i:end]...)
	}
}

// FlushDB clears the current Redis DB. Intended for seed/dev only — not exposed on HTTP routers.
...
func (redisCon RedisCon) HGetAll(key string) (res *redis.MapStringStringCmd, err error) {
