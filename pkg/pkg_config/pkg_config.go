...
type Cnf struct {
	...
	// JWT RSA key paths (private signs, public verifies)
...
func InitConfig(cnf Cnf) (config *ConfigInstance, err error) {
	...
	config.HTTPClient = &http.Client{
		Timeout: time.Duration(pkg_consts.HTTPClientTimeoutSec) * time.Second,
	}
...
func activityConnection(cnf Cnf, config *ConfigInstance) (err error) {
	...
	if cnf.ActivityDbDSN != "" {
		...
		config.ActivityCh = make(chan models.Activity, pkg_consts.ActivityChannelBuffer)
	}
	return
}
